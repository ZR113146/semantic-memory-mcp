...
# semantic-memory-mcp setup script (macOS + Linux)
...
BINARY_NAME="semantic-memory-mcp"
SOURCE_DIR="$HOME/.local/share/semantic-memory-mcp"
...
download_binary() {
    ...
    local asset="semantic-memory-mcp-${platform}.tar.gz"
...
build_from_source() {
    ...
    (cd "$SOURCE_DIR" && scripts/build.sh && cp build/c/semantic-memory-mcp "${INSTALL_DIR}/${BINARY_NAME}")
...
configure_claude() {
    ...
    printf "%s" "${BOLD}Configure Claude Code to use semantic-memory-mcp? [y/N] ${RESET}"
    read -r answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo ""
...
JSONEOF
    ...
    if command -v jq &>/dev/null; then
        ...
        if [ -f "$settings_file" ]; then
            local tmp
            tmp=$(mktemp)
            jq --argjson entry "$mcp_entry" '.mcpServers["semantic-memory-mcp"] = $entry' "$settings_file" > "$tmp"
            mv "$tmp" "$settings_file"
        else
            echo "{}" | jq --argjson entry "$mcp_entry" '.mcpServers["semantic-memory-mcp"] = $entry' > "$settings_file"
        fi
...
if os.path.exists(path):
    with open(path) as f:
        data = json.load(f)
data.setdefault('mcpServers', {})['semantic-memory-mcp'] = json.loads('$mcp_entry')
...
echo ""
echo "${BOLD}semantic-memory-mcp installer${RESET}"
echo ""
...
info "  rm -rf ~/.cache/semantic-memory-mcp/  # graph database"
