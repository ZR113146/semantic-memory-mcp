 ...
 *   Global:  $XDG_CONFIG_HOME/semantic-memory-mcp/config.json
 *            (falls back to ~/.config/semantic-memory-mcp/config.json)
 *   Project: {repo_root}/.semantic-memory.json
 *
...
cbm_userconfig_t *cbm_userconfig_load(const char *repo_path) {
    ...
    snprintf(global_path, sizeof(global_path), "%s/semantic-memory-mcp/config.json", cfg_fallback);
    ...
    if (repo_path && repo_path[0]) {
        ...
        snprintf(project_path, sizeof(project_path), "%s/.semantic-memory.json", repo_path);
