...
static const tool_def_t TOOLS[] = {
    ...
    {"memories_retrieve", "Retrieve task-relevant long-term memories with structured filters",
     ...
     "hit_count, decay, version, and supersedes info. Marks retrieved items as hits to feed "
     "decay/relevance. Use entity_key to pull a single decision timeline.",
    ...
    {"memory_feedback", "Record feedback on a retrieved memory and adjust its retrieval health",
     "Record outcome feedback on a retrieved memory: useful (boost future retrieval, slow "
     "decay), not_useful (demote), wrong (downgrade importance, fast decay), stale (archive "
     "candidate). Drives the recall-quality loop; pair with memories_retrieve.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
     "\"id\":{\"type\":\"string\"},\"feedback\":{\"type\":\"string\","
    ...
    {"memory_delete",
     ...
     "(undo a soft delete)",
     "Soft/hard/purge/restore a memory item. An ADR with code anchors can be archived but "
     "never physically purged — the purge redline keeps its history; use memory_update_status "
     "archived instead.",
     "{\"type\":\"object\",\"properties\":{\"project\":{\"type\":\"string\"},"
...
static char *memory_arg_string_dup(yyjson_doc *doc, const char *key) {
    ...
    return (v && yyjson_is_str(v)) ? heap_strdup(yyjson_get_str(v)) : NULL;
}
...
static char *handle_memory_feedback(cbm_mcp_server_t *srv, const char *args) {
    char *project = cbm_mcp_get_string_arg(args, "project");
    char *id = cbm_mcp_get_string_arg(args, "id");
