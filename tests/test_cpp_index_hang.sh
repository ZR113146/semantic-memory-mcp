...
BIN="$ROOT/build/c/semantic-memory-mcp"
...
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"; rm -f "$HOME/.cache/semantic-memory-mcp/$PROJ.db"* 2>/dev/null' EXIT
...
rm -f "$HOME/.cache/semantic-memory-mcp/$PROJ.db"* 2>/dev/null
