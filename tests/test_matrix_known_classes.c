...
static const char *MKC_ALL_EDGE_TYPES[] = {
...
static void mkc_diag(cbm_store_t *store, const char *project, const char *label) {
    if (!store) { fprintf(stderr, "    [MKC] %s: no graph DB\n", label); return; }
    char line[512] = {0};
...
static int mkc_edge(const MKC_File *files, int nfiles,
                    const char *edge_type, int floor,
    ...
    if (got < floor) {
        ...
        fprintf(stderr, "  [MKC] %s UNEXPECTED PASS %s=%d "
...
TEST(mkc_c3_rust_fn_attribute) {
    static const MKC_File f[] = {
        {"service.rs",
         "/* hypothetical local marker — real proc-macros live in separate crates,\n"
